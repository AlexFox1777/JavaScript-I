...
function sortByCarModel(){
    ...
    for (let i=0; i<carModels.length;i++) {
        ...
        for (let i = left; i < right; i++) {
            if (carModels[i].charAt(0) > carModels[i + 1].charAt(0)) {
                extrCar = carModels[i];
...
function carsOlderTwoK(){
    let cars = carYears();
    let oldCars = [];
    let oldCarsAmount = 0;
    for (let i=0; i< cars.length-1; i++){
        if(cars[i] < 2000){
            oldCars[oldCarsAmount] = cars[i];
            oldCarsAmount++;
        }
    }
    return  oldCars;
...
console.log(carsOlderTwoK().length + " cars are older than the year 2000: " +carsOlderTwoK());
